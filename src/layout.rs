//! Comet layout: one stateful model per output.
//!
//! The layout owns which windows exist and their stack (back → front).
//! [`State::apply_layout`] is the only writer of `Space` geometry and z-order.
//! A later tiling/scroll impl is a different type behind the same insert /
//! remove / activate / arrangement / visible operations, not another arm on
//! `placement(is_dialog, zone)`.
...
/// Per-output Comet model: stack (back → front) and the work zone.
#[derive(Debug)]
pub struct Layout {
    stack: Vec<WlSurface>,
    zone: Rectangle<i32, Logical>,
...
impl<BackendData: Backend + 'static> State<BackendData> {
    ...
    pub fn visible_surfaces(&self, output: &Output) -> HashSet<WlSurface> {
        self.layouts
            .get(output)
            .map(|layout| layout.visible(self.active_window.as_ref(), |s| self.parent_of(s)))
            .unwrap_or_default()
    }
}
